...
public class niteHandTracking extends PApplet {
...
public void draw()
{
  ...
  // draw the list
  if (pointDrawer._pointLists.size() <= 0)	return;
	...
	// draw the hand lists
	Iterator<Map.Entry> itrList = pointDrawer._pointLists.entrySet().iterator();
	while (itrList.hasNext()) {
		strokeWeight(2);
		stroke(pointDrawer._colorList[colorIndex % (pointDrawer._colorList.length - 1)]);
...
static public void main(final String args[]) {
	...
	int display=2;
  	...
  	String mainSketch="niteHandTracking";
...
class PointDrawer extends XnVPointControl {
	...
	HashMap _pointLists;
	int _maxPoints;
	public int[] _colorList;
	
	public PointDrawer(PApplet parentO, SimpleOpenNI parentKinectO) {
		_maxPoints = 30;
		_pointLists = new HashMap();
		parent=parentO;
		kinect=parentKinectO;
		_colorList = new int[] { parent.color(255f, 0f, 0f), parent.color(0f, 255f, 0f),
	               parent.color(0f, 0f, 255f), parent.color(255f, 255f, 0f) };
